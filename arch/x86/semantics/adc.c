...
void x86_insn_adc(struct x86_register_file* register_file, struct operand* dest, struct operand* src)
{
    ...
    int carry = CF(register_file) ? 1 : 0;

    if (sizeof(word) == dest->size) {
        int32_t c = (int32_t) WORD(dest) + WORD(src) + carry;
        WORD(dest) = c;
        ...
        int16_t c = (int16_t) BYTE(dest) + BYTE(src) + carry;
        BYTE(dest) = c;
    ...
    register_file->flags.af = (orig_dest_lsb + (BYTE(src) & 0x0F) + carry) > 0x0F;
}