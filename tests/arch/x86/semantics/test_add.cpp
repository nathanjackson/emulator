...
#include "arch/x86/semantics/operand.h"
...
TEST_F(x86_semantics_test, ADD_8bit)
{
    struct immediate_operand imm_op;
    struct register_operand reg_op;
    struct operand* dest = NULL;
    struct operand* src = NULL;

    // ADD AH, 1
    dest = make_register_operand(&reg_op, register_file, X86_REG_AH);
    src = make_u8_immediate(&imm_op, 1);
    ...
    PF(register_file) = 1;
    SF(register_file) = 1;
    x86_insn_add(register_file, dest, src);
...
TEST_F(x86_semantics_test, ADD_16bit)
{
    struct immediate_operand imm_op;
    struct register_operand reg_op;
    struct operand *dest = NULL;
    ...
    // ADD AX, 1
    dest = make_register_operand(&reg_op, register_file, X86_REG_AX);
    src = make_u16_immediate(&imm_op, 1);
